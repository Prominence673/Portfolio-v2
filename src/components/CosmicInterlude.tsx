...
function OrbitScene() {
  return (
    <>
      ...
      <div className="absolute bottom-[-22%] left-1/2 h-[38%] w-[82%] -translate-x-1/2">
        <div className="ambient-motion cosmic-orbit-clockwise absolute inset-0 rounded-[50%] border border-[#7dd3fc]/20" />
      </div>
      <div className="absolute bottom-[-8%] left-1/2 h-[54%] w-[64%] -translate-x-1/2">
...
function VoyageScene({ scrollProgress }: { scrollProgress: MotionValue<number> }) {
  const x = useTransform(scrollProgress, [0.38, 0.72], ["-18vw", "36vw"]);
  const y = useTransform(scrollProgress, [0.38, 0.72], [70, -55]);
...
export default function CosmicInterlude({ variant }: CosmicInterludeProps) {
  ...
  return (
    <section ref={ref} aria-hidden="true" className="relative h-[52vh] min-h-[360px] w-full overflow-hidden sm:h-[68vh] sm:min-h-[480px]">
